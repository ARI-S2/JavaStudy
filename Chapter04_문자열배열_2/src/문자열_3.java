...
public class 문자열_3 {
	...
	public static void main(String[] args) {
		String[] title={
			...
			int max = 0;
			for(int i = 0; i<title.length ;i++) {
				if(state[i].equals("상승")) { // 중요!! 상승에서만 비교!!
					if(max < modify[i]) {
						max = modify[i];
					}
				}			
			...
			for(int i =0; i<title.length; i++) {
				if(state[i].equals("상승") && modify[i]== max) { // 상태배열에서 상승인 것 중에 변경배열의 값이 max인 것
			...
			System.out.println("==============================================");
			// 노래명중에 가장 긴 제목의 노래와 가수명, 앨번명을 출력
			int max1 = 0;
			for(int i = 0; i<title.length ;i++) {
			...
			}System.out.println("임영웅 곡 :" + cnt);
			System.out.println("==============================================");
			
			// 검섹어를 입력받아 검색어가 포함된 노래명 출력
...
// 하나의 배열에서만 데이터를 가져오면 for each문 
// 두개 이상의 배열에서 데이터가 피룡하면 인덱스가 필요하므로 for문
}
	}
