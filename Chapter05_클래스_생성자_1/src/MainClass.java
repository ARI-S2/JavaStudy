/*
 * ** static 변수는 지역변수로 선언 불가능 => 클래스 내에 메소드 내에 선언 X
 * 	 클래스명.변수명 => 접근가능 (객체생성 전에 접근가능)
 * 
 * ** 멤버변수
 * 	  클래스 객체명.변수명 =? 접근가능 (new로 클래스 객체 생성 후 접근하능)
 * 
 * package => import 순서로!!
 *            ------ 
 *               ㄴ 외부에 있는 클래스를 불러올 때 
 * 
 * => 같은 파일 내에 있어도 class별로 따로 저장됨(컴파일이 따로 됨)
 * 
 * 
 */
//package , import 외에 클래스 위에 올라갈수 있는건 없다
/*
 *  클래스 : 
